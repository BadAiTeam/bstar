...
MODULE_VERSION = 'v9.3-cascade-fix'
...
class AdsPowerClient:
    ...
    def _get_default_group_id(self):
        ...
        Priority:
          1. Use explicitly configured group_id (from constructor, env var, or proxy.json)
    ...
    def create_profile(self, profile_config):
        ...
        Strategy (avoids profile limit and rate limiting):
          1. If profile_id configured in proxy.json → use it directly
        ...
        logger.error(
            ...
            f"Solusi: Tambahkan 'profile_id' di proxy.json.\n"
            f"  1. Buka AdsPower, buat profil baru secara manual\n"
            f"  2. Salin ID profil dari daftar profil\n"
            f"  3. Tambahkan di proxy.json: \"profile_id\": \"ID_PROFIL\""
        )
        return None
...
class AntiDetectManager:
    ...
    def __init__(self, mode="antidetect", browser_type="adspower", api_key=None, profile_id=None, group_id=None):
        """
        Args:
        ...
        self.api_key = api_key or os.environ.get('ADSPOWER_API_KEY', '')
        self.profile_id = profile_id or os.environ.get('ADSPOWER_PROFILE_ID', '')
        self.group_id = group_id or os.environ.get('ADSPOWER_GROUP_ID', '')
        self._client = None
        self._sessions = []
    ...
    def _init_client(self):
        ...
        if self.browser_type == 'adspower':
            self._client = AdsPowerClient(api_key=self.api_key, profile_id=self.profile_id, group_id=self.group_id)
