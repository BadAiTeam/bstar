...
# ====================================================================
# AdsPower Auto-Install & Launch
# ====================================================================

ADSPOWER_BINARY = 'adspower_global'
ADSPOWER_DEB_URL_GDOWN = '1RuAWUkwKQEd9HUYT9VhCyvrgPsyYQzg5'
ADSPOWER_DEB_FILENAME = 'AdsPower-Global-8.6.3-x64.deb'
ADSPOWER_DEFAULT_API_KEY = '0a9a0ceae5ea9e56065395e58ce43ace0092eda5d96b6ecd'


def _is_adspower_installed():
    """Check if AdsPower binary exists in PATH."""
    return shutil.which(ADSPOWER_BINARY) is not None


def _is_adspower_running():
    """Check if AdsPower process is already running."""
    try:
        result = subprocess.run(
            ['pgrep', '-f', ADSPOWER_BINARY],
...
def _install_adspower():
    """
    Download and install AdsPower from Google Drive via gdown.
    Returns True if installation succeeded.
    """
    deb_path = os.path.join(tempfile.gettempdir(), ADSPOWER_DEB_FILENAME)

    # Step 1: Check if deb already downloaded
    if not os.path.exists(deb_path):
        print(f"[AdsPower] Downloading {ADSPOWER_DEB_FILENAME} via gdown...")
        # Ensure gdown is installed
        try:
            import gdown
        except ImportError:
            print("[AdsPower] gdown not found, installing...")
            subprocess.check_call(
        ...
        try:
            gdown.download(
                id=ADSPOWER_DEB_URL_GDOWN,
                output=deb_path,
                quiet=False,
            )
            print(f"[AdsPower] Downloaded to {deb_path}")
        except Exception as e:
            print(f"[AdsPower] Download failed: {e}")
            # Fallback: try with gdown CLI
            try:
                subprocess.check_call(
                    ['gdown', ADSPOWER_DEB_URL_GDOWN, '-O', deb_path],
                    timeout=300,
                )
                print(f"[AdsPower] Downloaded via CLI to {deb_path}")
            except Exception as e2:
                print(f"[AdsPower] CLI download also failed: {e2}")
                return False
    else:
        print(f"[AdsPower] DEB already exists at {deb_path}, skipping download")

    # Step 2: Install the .deb package
    if not os.path.exists(deb_path):
        print(f"[AdsPower] ERROR: DEB file not found at {deb_path}")
        return False

    print(f"[AdsPower] Installing {deb_path}...")
    try:
        subprocess.check_call(
            ['dpkg', '-i', deb_path],
            stdout=subprocess.DEVNULL,
        ...
        print("[AdsPower] dpkg install done")
    except subprocess.CalledProcessError:
        # Fix broken deps then retry
        print("[AdsPower] Fixing broken dependencies...")
        try:
            subprocess.check_call(
                ['apt-get', 'install', '-f', '-y'],
                stdout=subprocess.DEVNULL,
            ...
            print("[AdsPower] dpkg install succeeded after dependency fix")
        except subprocess.CalledProcessError as e:
            print(f"[AdsPower] Installation failed: {e}")
            return False

    # Step 3: Verify installation
    if _is_adspower_installed():
        print(f"[AdsPower] ✓ Installed successfully: {shutil.which(ADSPOWER_BINARY)}")
        return True
    else:
        print("[AdsPower] ERROR: Binary not found after installation")
        return False


def _launch_adspower(api_key=None):
    """
    Launch AdsPower in background with headless mode.
    Returns True if launch succeeded.
    """
    if _is_adspower_running():
        print("[AdsPower] Already running, skipping launch")
        return True

    key = api_key or ADSPOWER_DEFAULT_API_KEY
    display = os.environ.get('DISPLAY', ':99')

    print(f"[AdsPower] Launching in background (DISPLAY={display}, headless=true)...")
    try:
        proc = subprocess.Popen(
            [ADSPOWER_BINARY, '--headless=true', '--no-sandbox', f'--api-key={key}'],
        ...
        time.sleep(3)
        if _is_adspower_running():
            print(f"[AdsPower] ✓ Running in background (PID: {proc.pid})")
            return True
        else:
            print("[AdsPower] WARNING: Process exited shortly after launch")
            # Try to get error output
            retcode = proc.poll()
            print(f"[AdsPower] Exit code: {retcode}")
            return False
    except FileNotFoundError:
        print(f"[AdsPower] ERROR: {ADSPOWER_BINARY} not found")
        return False
    except Exception as e:
        print(f"[AdsPower] Launch failed: {e}")
        return False


def ensure_adspower(api_key=None):
    """
    Ensure AdsPower is installed, installed+launched, or already running.
    This is the main entry point called from main().
    
    Flow:
      1. Check if running → return True
      2. Check if installed → if not, download & install
      3. Launch in background
      4. Wait for API to become ready (up to 30s)
    
    Returns True if AdsPower is ready to use.
    """
    key = api_key or ADSPOWER_DEFAULT_API_KEY

    # Step 1: Already running?
    if _is_adspower_running():
        print("[AdsPower] ✓ Already running")
        return True

    # Step 2: Installed?
    if not _is_adspower_installed():
        print("[AdsPower] Not installed. Starting auto-install...")
        if not _install_adspower():
            print("[AdsPower] ✗ Installation failed")
            return False
    else:
        print(f"[AdsPower] ✓ Binary found: {shutil.which(ADSPOWER_BINARY)}")

    # Step 3: Launch
    if not _launch_adspower(api_key=key):
        print("[AdsPower] ✗ Launch failed")
        return False

    # Step 4: Wait for API ready (poll /status endpoint)
    import requests as _req
    port = 50325
    api_url = f'http://127.0.0.1:{port}/status'
    print(f"[AdsPower] Waiting for API to become ready on port {port}...")
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if data.get('code') == 0 or data.get('status') == 'ok':
                    print(f"[AdsPower] ✓ API ready after {(attempt + 1) * 2}s")
                    return True
        except Exception:
            pass
        if attempt % 3 == 2:
            print(f"[AdsPower]   Still waiting... ({(attempt + 1) * 2}s)")

    print("[AdsPower] ⚠ API did not respond within 30s, but process is running — continuing anyway")
    return True
...
def main():
    ...
    if args.mode == 'patchright':
        ...
        log.info('', 'Anti-detect mode: ensuring AdsPower is installed and running...')
        adspower_key = os.environ.get('ADSPOWER_API_KEY', ADSPOWER_DEFAULT_API_KEY)
        if ensure_adspower(api_key=adspower_key):
            log.ok('', 'AdsPower is ready')
        else:
            log.warn('', 'AdsPower auto-install/launch failed — will fall back to Patchright if needed')
